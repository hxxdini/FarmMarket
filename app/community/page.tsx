...
function CommunityPageContent() {
  ...
  async function handlePostAdvice() {
    ...
    if (res.ok) {
      ...
      setAdviceError(error || "Only administrators, verified experts, and extension officers can post advice")
    }
  }
  ...
  async function handlePostAlert() {
    ...
    if (res.ok) {
      ...
      const { error } = await res.json()
      setAlertError(error || "Only administrators, verified experts, and extension officers can post alerts")
    }
  }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 sm:gap-6 lg:gap-8">
          ...
          <div className="lg:col-span-3">
            <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4 sm:space-y-6">
              <TabsList className="flex w-full overflow-x-auto scrollbar-hide gap-1 sm:gap-2 pb-2 sm:pb-0">
                <TabsTrigger value="discussions" className="text-xs sm:text-sm px-2 sm:px-3 py-2 whitespace-nowrap flex-shrink-0">Discussions</TabsTrigger>
                <TabsTrigger value="ask" className="text-xs sm:text-sm px-2 sm:px-3 py-2 whitespace-nowrap flex-shrink-0">Ask</TabsTrigger>
                <TabsTrigger value="experts" className="text-xs sm:text-sm px-2 sm:px-3 py-2 whitespace-nowrap flex-shrink-0">Experts</TabsTrigger>
                {isVerifiedExpert && (
                  <TabsTrigger value="share-advice" className="text-xs sm:text-sm px-2 sm:px-3 py-2 whitespace-nowrap flex-shrink-0">Advice</TabsTrigger>
                )}
                <TabsTrigger value="report-alert" className="text-xs sm:text-sm px-2 sm:px-3 py-2 whitespace-nowrap flex-shrink-0">Alerts</TabsTrigger>
                <TabsTrigger value="share-success" className="text-xs sm:text-sm px-2 sm:px-3 py-2 whitespace-nowrap flex-shrink-0">Success</TabsTrigger>
              </TabsList>

              <TabsContent value="discussions" className="space-y-3 sm:space-y-4">
                    ...
                    <CardHeader className="pb-3 px-4 sm:px-6">
                      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 sm:gap-0">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          {discussion.crop ? (
                            <Badge variant="secondary" className="text-xs">{discussion.crop}</Badge>
                          ) : null}
                          {discussion.hasExpertReply && (
                            <Badge className="bg-green-100 text-green-800 text-xs">Expert Reply</Badge>
                          )}
                        </div>
                      </div>
                      <CardTitle className="text-base sm:text-lg hover:text-green-600 cursor-pointer leading-tight">{discussion.title}</CardTitle>
                    </CardHeader>

                    <CardContent className="px-4 sm:px-6">
                      <div
                        className="text-gray-600 mb-4 line-clamp-2 text-sm sm:text-base"
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-0">
                        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs sm:text-sm text-gray-600">{discussion.author}</span>
                        ...
                        <div className="flex items-center gap-4 text-xs sm:text-sm text-gray-500">
                          ...
                          >
                            <ThumbsUp className={`h-3 w-3 sm:h-4 sm:w-4 mr-1 ${discussion.isLiked ? 'text-green-600 fill-current' : ''}`} />
                            {discussion.likes}
                          </button>
              ...
              <TabsContent value="ask" className="space-y-4 sm:space-y-6">
                <Card>
                  <CardHeader className="px-4 sm:px-6">
                    <CardTitle className="text-lg sm:text-xl">Ask the Community</CardTitle>
                    <CardDescription className="text-sm sm:text-base">Get help from fellow farmers and agricultural experts</CardDescription>
                  </CardHeader>
              ...
              <TabsContent value="experts" className="space-y-3 sm:space-y-4">
                      ...
                      <CardContent className="pt-4 sm:pt-6 px-4 sm:px-6">
                        <div className="flex items-start gap-3 sm:gap-4">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs sm:text-sm text-gray-600">{expert.title}</p>
                            <p className="text-xs sm:text-sm text-green-600">{expert.specialization}</p>
                            <p className="text-xs text-gray-500 mt-1">{expert.location}</p>
                            <div className="flex items-center mt-2 gap-3 sm:gap-4 text-xs sm:text-sm">
                              <span>⭐ {expert.rating}</span>
                              <span>{expert.responses} responses</span>
                            </div>
                            <Button size="sm" className="mt-2 sm:mt-3 text-xs sm:text-sm">
                              Ask Question
                            </Button>
                          </div>
                        </div>
              ...
              {isVerifiedExpert && (
              <TabsContent value="share-advice" className="space-y-4 sm:space-y-6">
                <Card>
                  <CardHeader className="px-4 sm:px-6">
                    <CardTitle className="text-lg sm:text-xl">Share Expert Advice</CardTitle>
                    <CardDescription className="text-sm sm:text-base">Provide best practices and guidance for farmers</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-4 px-4 sm:px-6">
                    {adviceError && (
                    ...
                    <p className="text-xs text-gray-500 text-center">Only administrators, verified experts, and extension officers can publish advice. Submissions go through moderation.</p>
                  </CardContent>
                </Card>
              </TabsContent>
              )}

              <TabsContent value="report-alert" className="space-y-4 sm:space-y-6">
                <Card>
                  <CardHeader className="px-4 sm:px-6">
                    <CardTitle className="text-lg sm:text-xl">Report Important Alert</CardTitle>
                    <CardDescription className="text-sm sm:text-base">Notify the community about urgent issues (diseases, weather, market risks)</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-4 px-4 sm:px-6">
                    {alertError && (
                    ...
                    <p className="text-xs text-gray-500 text-center">Only administrators, verified experts, and extension officers can publish alerts. Submissions go through moderation.</p>
                  </CardContent>
                </Card>
              </TabsContent>

              <TabsContent value="share-success" className="space-y-4 sm:space-y-6">
                <Card>
                  <CardHeader className="px-4 sm:px-6">
                    <CardTitle className="text-lg sm:text-xl">Share Success Story</CardTitle>
                    <CardDescription className="text-sm sm:text-base">Inspire others with what worked well for you</CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-4 px-4 sm:px-6">
                    <div>
                      <label className="text-sm font-medium mb-2 block">Title</label>
          ...
          <div className="space-y-4 sm:space-y-6 order-first lg:order-last">
            <Card className="border-green-200">
              <CardHeader className="px-4 sm:px-6">
                <CardTitle className="text-base sm:text-lg flex items-center gap-2">
                  <BookOpen className="h-4 w-4 sm:h-5 sm:w-5 text-green-700" />
                  Knowledge Base
                </CardTitle>
              </CardHeader>
            ...
            <Card>
              <CardHeader className="px-4 sm:px-6">
                <CardTitle className="text-base sm:text-lg">Popular Topics</CardTitle>
              </CardHeader>
              <CardContent className="px-4 sm:px-6">
                {isLoadingStats ? (
                  ...
                  <div className="space-y-2">
                    {popularTopics.length > 0 ? (
                      popularTopics.map((item, index) => (
                        <div key={index} className="flex justify-between items-center py-1">
                          <span className="text-xs sm:text-sm text-gray-600">{item.topic}</span>
              ...
              <CardContent className="px-4 sm:px-6">
                {isLoadingStats ? (
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between">
                      <span className="text-xs sm:text-sm text-gray-600">Questions Answered</span>
                      <span className="font-semibold text-xs sm:text-sm">
                        {communityStats?.stats?.questionsAnswered?.toLocaleString() || "0"}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-xs sm:text-sm text-gray-600">Expert Responses</span>
                      <span className="font-semibold text-xs sm:text-sm">
